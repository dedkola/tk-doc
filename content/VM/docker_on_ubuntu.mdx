...
<Alert variant="info">
  **Docker Desktop vs Docker Engine**: Docker Desktop runs containers inside a
  lightweight VM and includes additional tools like Docker Compose, Docker
  Build, and a GUI. Docker Engine is the core runtime that can run standalone.
  Both can be installed side-by-side on the same machine.
</Alert>
...
<Alert variant="warning">
  **Important**: Images, containers, volumes, and networks stored in
...
<Alert variant="info">
  **Convenience Script Details**: The `get.docker.com` script automatically
...
<Alert variant="warning">
  **Security Note**: Adding users to the `docker` group grants privileges
